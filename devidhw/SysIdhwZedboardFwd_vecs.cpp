/**
  * \file SysIdhwZedboardFwd_vecs.cpp
  * SPI command forwarder based on ZedBoard system (implementation of vectors)
...
/******************************************************************************
 class SysIdhwZedboardFwd::VecVTarget
 ******************************************************************************/

uint SysIdhwZedboardFwd::VecVTarget::getIx(
			const string& sref
		) {
	...
	else if (s == "zedb") return ZEDB;

	return(0);
};

string SysIdhwZedboardFwd::VecVTarget::getSref(
			const uint ix
		) {
	...
	else if (ix == ZEDB) return("zedb");

	return("");
};

void SysIdhwZedboardFwd::VecVTarget::fillFeed(
			Feed& feed
		) {
	feed.clear();

	set<uint> items = {AXS2_PHI,AXS2_THETA,DCX3,DCX3_AXS2_PHI,DCX3_AXS2_THETA,DCX3_ICM2,DCX3_TAU2,ICM2,ZEDB};
