...
/******************************************************************************
 class CtrIdhwZedbAlua::CmdValget
 ******************************************************************************/

CtrIdhwZedbAlua::CmdValget::CmdValget() : Cmd(0x03, VecVCommand::VALGET, Cmd::VecVRettype::IMMSNG) {
	returnSpeccallback = NULL;
	argReturnSpeccallback = NULL;
};

void CtrIdhwZedbAlua::CmdValget::setReturnSpeccallback(
...
void CtrIdhwZedbAlua::CmdValget::returnToCallback() {
...
/******************************************************************************
 class CtrIdhwZedbAlua::VecVCommand
 ******************************************************************************/

utinyint CtrIdhwZedbAlua::VecVCommand::getTix(
			const string& sref
		) {
...
string CtrIdhwZedbAlua::VecVCommand::getSref(
			const utinyint tix
		) {
...
void CtrIdhwZedbAlua::VecVCommand::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class CtrIdhwZedbAlua::VecVError
 ******************************************************************************/

utinyint CtrIdhwZedbAlua::VecVError::getTix(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CtrIdhwZedbAlua::VecVError::getSref(
			const utinyint tix
		) {
...
string CtrIdhwZedbAlua::VecVError::getTitle(
			const utinyint tix
		) {
...
/******************************************************************************
 class CtrIdhwZedbAlua::VecVReg
 ******************************************************************************/

utinyint CtrIdhwZedbAlua::VecVReg::getTix(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CtrIdhwZedbAlua::VecVReg::getSref(
			const utinyint tix
		) {
...
string CtrIdhwZedbAlua::VecVReg::getTitle(
			const utinyint tix
		) {
...
void CtrIdhwZedbAlua::VecVReg::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class CtrIdhwZedbAlua
 ******************************************************************************/

CtrIdhwZedbAlua::CtrIdhwZedbAlua(
			XchgIdhw* xchg
...
utinyint CtrIdhwZedbAlua::getTixVCommandBySref(
			const string& sref
		) {
	return VecVCommand::getTix(sref);
};

string CtrIdhwZedbAlua::getSrefByTixVCommand(
			const utinyint tixVCommand
		) {
	return VecVCommand::getSref(tixVCommand);
};

void CtrIdhwZedbAlua::fillFeedFCommand(
			Feed& feed
		) {
	VecVCommand::fillFeed(feed);
};

Cmd* CtrIdhwZedbAlua::getNewCmd(
			const utinyint tixVCommand
		) {
	Cmd* cmd = NULL;
...
Cmd* CtrIdhwZedbAlua::getNewCmdAdd(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::ADD, Cmd::VecVRettype::VOID);

	cmd->addParInv("aTixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
	cmd->addParInv("bTixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
	cmd->addParInv("cTixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
...
void CtrIdhwZedbAlua::add(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "add", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdSub(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::SUB, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::sub(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "sub", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdMult(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::MULT, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::mult(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "mult", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdDiv(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::DIV, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::div(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "div", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdPow(
			const utinyint aTixVReg
			, const utinyint exp
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::POW, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::pow(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "pow", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdLgcand(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::LGCAND, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::lgcand(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "lgcand", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdLgcor(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::LGCOR, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::lgcor(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "lgcor", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdLgcxor(
			const utinyint aTixVReg
			, const utinyint bTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::LGCXOR, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::lgcxor(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "lgcxor", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdLgcnot(
			const utinyint aTixVReg
			, const utinyint cTixVReg
		) {
	Cmd* cmd = new Cmd(0x03, VecVCommand::LGCNOT, Cmd::VecVRettype::VOID);
...
void CtrIdhwZedbAlua::lgcnot(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "lgcnot", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

Cmd* CtrIdhwZedbAlua::getNewCmdValset(
	...
	Cmd* cmd = new Cmd(0x03, VecVCommand::VALSET, Cmd::VecVRettype::VOID);

	cmd->addParInv("r1TixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
	cmd->addParInv("r1Val", Par::VecVType::VBLOB, NULL, NULL, NULL, 8);
	cmd->addParInv("r2TixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
	cmd->addParInv("r2Val", Par::VecVType::VBLOB, NULL, NULL, NULL, 8);
	cmd->addParInv("r3TixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
	cmd->addParInv("r3Val", Par::VecVType::VBLOB, NULL, NULL, NULL, 8);
	cmd->addParInv("r4TixVReg", Par::VecVType::TIX, CtrIdhwZedbAlua::VecVReg::getTix, CtrIdhwZedbAlua::VecVReg::getSref, CtrIdhwZedbAlua::VecVReg::fillFeed);
...
void CtrIdhwZedbAlua::valset(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "valset", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	delete cmd;

	if (msg != "") throw(DbeException(msg));
};

CtrIdhwZedbAlua::CmdValget* CtrIdhwZedbAlua::getNewCmdValget(
			const utinyint r1TixVReg
			, const utinyint r2TixVReg
			, const utinyint r3TixVReg
			, const utinyint r4TixVReg
...
void CtrIdhwZedbAlua::valget(
	...
	if (cmd->err.tixDbeVAction != 0x00) msg = cmd->err.getMessage("alua", "valget", cmd->cref, VecVError::getSref(cmd->err.tixVError), VecVError::getTitle(cmd->err.tixVError), true, true);
	else {
...
string CtrIdhwZedbAlua::getSrefByTixVError(
			const utinyint tixVError
		) {
	return VecVError::getSref(tixVError);
};

string CtrIdhwZedbAlua::getTitleByTixVError(
			const utinyint tixVError
		) {
	return VecVError::getTitle(tixVError);
};

Err CtrIdhwZedbAlua::getNewErr(
			const utinyint tixVError
		) {
	Err err;
...
Err CtrIdhwZedbAlua::getNewErrInvalid() {
...
Err CtrIdhwZedbAlua::getNewErrMismatch() {
	Err err(VecDbeVAction::ERR, VecVError::MISMATCH);

	return err;
};

Err CtrIdhwZedbAlua::getNewErrSize() {
